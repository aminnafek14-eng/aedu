  ...
  return (
    <div style={S.page}>
      <div style={S.header}>
        ...
        {student && <p style={S.welcome}>Selamat datang, <strong>{student.full_name}</strong>! 👋</p>}
      </div>
      ...
      {banners.length > 0 && (
        <div style={S.bannerWrap}>
          <div style={S.bannerTrack}>
            {banners.map((b, i) => (
              <div key={b.id} style={{ ...S.bannerSlide, display: i === bannerIdx ? 'flex' : 'none' }}
                onClick={() => b.link_url && window.open(b.link_url, '_blank')}>
                {b.img_url
                  ? <img src={b.img_url} alt={b.title} style={S.bannerImg} />
                  : <div style={S.bannerPlaceholder}><span style={{ fontSize: 32 }}>📢</span><span style={{ fontWeight: 700, fontSize: 16, marginTop: 8 }}>{b.title}</span></div>}
              ...
              {folders.map(f => (
                <div key={f.id} style={S.folderCard} onClick={() => router.push(`/folder/${f.id}`)}>
                  <div style={S.folderImg}>
                    {f.img_url ? <img src={f.img_url} alt={f.name} style={S.folderImgFill} /> : <span style={{ fontSize: 38 }}>{f.emoji || '📁'}</span>}
                  </div>
                  <div style={S.folderFoot}><div style={S.folderName}>{f.name}</div></div>
                </div>
              ))}
            </div>}
      </div>
      <div style={S.adminLink}><a href="/admin" style={S.adminTiny}>⚙️ Admin</a></div>
    </div>
  )
}
...
const S: Record<string, React.CSSProperties> = {
  ...
  spinner: { width: 36, height: 36, border: '3px solid #E2E8F0', borderTopColor: '#4F46E5', borderRadius: '50%' },
  ...
  bannerSlide: { width: '100%', minHeight: 160, flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' },
  ...
  folderCard: { background: 'white', borderRadius: 16, border: '1px solid #E2E8F0', overflow: 'hidden', cursor: 'pointer', aspectRatio: '1/1', display: 'flex', flexDirection: 'column', boxShadow: '0 2px 8px rgba(0,0,0,0.07)' },
